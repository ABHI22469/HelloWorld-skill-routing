...
An internship test project — a practice starter exercise, not real product
code. It lives in a sandbox / practice repo, agreed with the reviewer, and
should never be copied into a product application path.
...
## Coding convention: the `com.helloworld` package

The Developer and Tester skills both produce Python source files. Every file
they create belongs to the `com.helloworld` package. Since Python doesn't have a
`package` statement like Java, this is represented two ways in every file:

1. The file lives under the folder path `com/helloworld/...` (with an empty
   `__init__.py` in `com/` and `com/helloworld/`).
2. The file starts with the header comment `# package: com.helloworld`.
